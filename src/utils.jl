...
function granule_from_file(filename::AbstractString)
    ...
    empty_bbox = (min_x = missing, min_y = missing, max_x = missing, max_y = missing)
    if startswith(name, "ATL")
        info = icesat2_info(name)
        return ICESat2_Granule(info.type, name, filename, "", empty_bbox, info)
    elseif startswith(name, "GEDI")
        info = gedi_info(name)
        return GEDI_Granule(info.type, name, filename, "", empty_bbox, info)
    elseif startswith(name, "GLAH")
        info = icesat_info(name)
        return ICESat_Granule(info.type, name, filename, "", empty_bbox, info)
    else
